...
mc.LOO.KNN = function(points, classes) {
    n = dim(points)[1]
    looY = rep(0, n-1) #n-1, потому что один элемент всегда будет отсутствовать из выборки
...
mc.draw.KNN = function(points, classes, colors, k, xlim, ylim, step) {
    ...
    for (x in seq(xlim[1], xlim[2], step)) {
        for (y in seq(ylim[1], ylim[2], step)) {
            u = c(round(x, 1), round(y, 1)) #избегаем случаев 0.1 + 0.2 = 0.3000000004
            if (any(apply(points, 1, function(v) all(v == u)))) next #не классифицировать известные точки
